...
std::shared_ptr<robot::Robot> BlueConfiguration::buildRobot()
{
    ...
    std::unique_ptr<wisco::io::IRotationSensor> pros_strafe_rotation_sensor{std::make_unique<pros_adapters::ProsRotation>(pros_strafe_rotation)};
    std::unique_ptr<wisco::io::IDistanceTrackingSensor> strafe_tracking_wheel{std::make_unique<wisco::hal::TrackingWheel>(pros_strafe_rotation_sensor, ODOMETRY_STRAFE_RADIUS)};
